...
const EmployerNotification = require("../models/employer_notification");
      ...
      if (!internship) {
        ...
        return res
          ...
          .send({
            message: "application sent!",
            status: "Pending!",
            applicationId: application_._id,
          });
      }
    }
    if (
      applyingFor.toLowerCase() === "fresherjob" ||
      applyingFor.toLowerCase() === "fresher job"
    ) {
...
exports.approve = async (req, res) => {
  try {
    ...
    if (application.employer.toString() === req.user._id.toString()) {
      ...
      application.applicantReceiveNote =
        "you have been shortlisted for the elementary-test round!";
      await application.save();
      ...
      return res
        ...
        .send({
          message:
            "you cannot approve the application, you did not create the job/ internship!",
        });
    }
  } catch (e) {
    return res.status(400).send({ message: "something went wrong!" });
...
exports.reject = async (req, res) => {
  try {
    ...
    if (application.employer.toString() === req.user._id.toString()) {
      ...
      application.applicantReceiveNote =
        "sorry, you have not been shortlisted for the elementary-test round!";
      try {
        const student = await Student.findById(application.applicantId);
        if (student) {
          const notification = new StudentNotification({
      ...
      return res
        .status(200)
        .send({ message: "application has been rejected!" });
    } else {
      return res
        .status(200)
        .send({
...
exports.employer_getPendingInternships = async (req, res) => {
  try {
    ...
    if (!applications || applications.length === 0) {
      return res
        .status(200)
        .send({ message: "no pending internship applications" });
    }
    return res.status(200).send(applications);
  } catch (e) {
    console.log(e);
...
exports.employer_getPendingFresherJobs = async (req, res) => {
  try {
    ...
    if (!applications || applications.length === 0) {
      return res
        .status(200)
        .send({ message: "no pending fresher job applications" });
    }
    return res.status(200).send(applications);
  } catch (e) {
    console.log(e);
