...
exports.signup = async (req, res) => {
  ...
  if (password !== passwordConfirmation) {
    return res.json({
      error: "passwordConfirmation field is missing or Password dosen't match",
    });
  }
  if (
    !companyName ||
...
exports.signin = async (req, res) => {
  try {
    ...
    let savedUser;
    try {
      savedUser = await Employer.findByCredentials(
        req.body.email,
        req.body.password
      );
    } catch (e) {
    ...
    return res
      .status(200)
      .json({ token, user: { _id, personName, email, contact, companyName } });
  } catch (e) {
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ user: { _id, personName, email, contact, companyName } });
  } catch (e) {
